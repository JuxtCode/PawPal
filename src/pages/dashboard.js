...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "users")); // Fetching user data from Firestore
        const users = querySnapshot.docs.map(doc => doc.data());
        setUserData(users); // Set fetched user data into state
      } catch (error) {
    ...
    <div style={dashboardContainerStyle}>
      ...
      <div style={videoWrapperStyle}>
        <video autoPlay loop muted style={videoStyle}>
          <source src="https://videos.pexels.com/video-files/1182756/1182756-sd_640_360_25fps.mp4"
            type="video/mp4" />
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StyledCard>
              <CardContent>
                ...
                <Button variant="contained" color="primary" fullWidth style={cardButtonStyle} onClick={handleViewDogs}>
                  View Dogs
                </Button>
              </CardContent>
            </StyledCard>
